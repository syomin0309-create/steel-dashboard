...
<div class="spc-statsbar" style="margin-top:10px">
  ...
  <div class="spc-cell"><div class="spc-lbl">最大值</div><div class="spc-val" style="font-family:'Space Mono',monospace;font-size:.85rem;color:#c8dde8;font-weight:700">{spc_max:.4f}</div></div>
            ...
            with ch1:
                ...
                for i in range(bins):
                    ...
                    if is_both or is_upper:
                        out = out or (lo >= usl2)
                    bar_colors.append("rgba(255,59,59,0.75)" if out else "rgba(0,180,220,0.55)")
                ...
                if spc_std > 0 and show_curve2:
                    ...
                    fig_h.add_trace(go.Scatter(
                        ...
                        line=dict(color='#f5a623', width=2.5), name='常態曲線'
                    ))
                ...
                for xv, lb, cl, dk in spec_lines:
                    fig_h.add_vline(x=xv, line_dash=dk, line_color=cl, line_width=1.8,
                        annotation=dict(text=lb, font=dict(color=cl, size=10),
                            bgcolor="rgba(10,16,24,0.85)", bordercolor=cl, borderwidth=1, borderpad=3))

                fig_h.update_layout(
                    title=dict(text=f"【{spc_param}】 直方圖 · 常態分佈",
                        font=dict(color="#5a7a8e", size=12, family="Space Mono"), x=0),
                    height=420, plot_bgcolor="#0c1220", paper_bgcolor="#101826",
                    font=dict(color="#c8dde8"), bargap=0,
                    xaxis=dict(gridcolor="#1c2e42", tickfont=dict(size=9, color="#5a7a8e"),
                        title=dict(text=spc_param, font=dict(color="#5a7a8e", size=10))),
                    yaxis=dict(gridcolor="#1c2e42", tickfont=dict(size=9, color="#5a7a8e"),
                        title=dict(text="次數 (Frequency)", font=dict(color="#5a7a8e", size=10))),
                    legend=dict(bgcolor="rgba(12,18,32,0.8)", bordercolor="#1c2e42", borderwidth=1,
            ...
            with ch2:
                ...
                fig_p.update_layout(
                    title=dict(text="規格符合率", font=dict(color="#5a7a8e", size=12, family="Space Mono"), x=0),
                    height=420, paper_bgcolor="#101826", plot_bgcolor="#101826",
