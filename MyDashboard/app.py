...
meta_cols = {
    '產出鋼捲號碼', '試驗等級', '生產日期',
    '訂單厚度', '訂單寬度', '熱軋材質', '產品規格代碼',
    '生產年月'
}
available_params = [col for col in raw_df.columns if col not in meta_cols]
    ...
    if len(months_in_data) < 2:
        ...
        for param in numeric_params:
            ...
            for month in months_in_data:
                ...
                m_mean = mdf.mean()
                if g_std == 0:
                    row[month] = ("sig-green", "●", f"{m_mean:.2f}")
                    continue
                z = abs(m_mean - g_mean) / g_std
                if z < 0.5:
                    row[month] = ("sig-green",  "●", f"{m_mean:.2f}")
                elif z < 1.5:
                    row[month] = ("sig-yellow", "▲", f"{m_mean:.2f}")
                else:
                    row[month] = ("sig-red",    "✖", f"{m_mean:.2f}")
            rows.append(row)

        st.markdown("""
        <div style="margin-bottom:12px; font-size:13px; display:flex; gap:24px;">
            <span><span class="sig-green">●</span> 正常（偏移 &lt; 0.5σ）</span>
            <span><span class="sig-yellow">▲</span> 輕微偏移（0.5 ~ 1.5σ）</span>
            <span><span class="sig-red">✖</span> 顯著異常（&gt; 1.5σ）</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        for row in rows:
            ...
            for month in months_in_data:
                css, icon, val = row.get(month, ("sig-gray", "—", "—"))
                tds += f'<td><span class="{css}">{icon}</span><br><small>{val}</small></td>'
        ...
        for row in rows:
            for month in months_in_data:
                css, icon, val = row.get(month, ("sig-gray", "", ""))
                if css == "sig-red":
                    red_items.append(f"**{month}**　【{row['參數']}】　均值 {val}")
        if red_items:
            st.error("🔴 **需要重點追蹤的顯著異常項目：**\n\n" + "\n\n".join(f"- {x}" for x in red_items))
        else:
...
with tab2:
    if '生產年月' not in df.columns:
        st.info("ℹ️ 無生產年月欄位，無法繪製趨勢圖。")
    else:
        fig_line = px.line(
            df, x="產出鋼捲號碼", y=selected_param,
            color="生產年月",
        ...
        st.caption("💡 紅色虛線為 ±3σ 管制界限（SPC），各月份以不同顏色區分。")
...
with tab3:
    ...
    if '生產年月' not in df.columns:
        ...
        month_summary = (
            ...
            .agg(鋼捲數='count', 平均值='mean', 標準差='std',
                 最小值='min', 最大值='max', 中位數='median')
            .round(3)
            .reset_index()
...
with dl2:
    if '生產年月' in df.columns:
        month_summary_all = (
            df.groupby('生產年月')[available_params]
            .agg(['mean', 'std', 'min', 'max', 'count'])
            .round(3)
        )
        st.download_button(
            label="📊 下載各月統計摘要 (CSV)",
            data=month_summary_all.to_csv().encode('utf-8-sig'),
