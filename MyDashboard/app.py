    ...
    if filtered_df.empty:
        ...
        with tab_comparison:
            ...
            if period_a_months and period_b_months:
                # 對比規格篩選
                st.markdown("**精確規格篩選 (可選)：**")
                
                comp_filter_col1, comp_filter_col2, comp_filter_col3, comp_filter_col4 = st.columns(4)
                
                with comp_filter_col1:
                    comp_thick = st.multiselect("訂單厚度", 
                                               sorted(filtered_df['訂單厚度'].unique() if '訂單厚度' in filtered_df.columns else []),
                                               key="comp_thick")
                
                with comp_filter_col2:
                    comp_width = st.multiselect("訂單寬度",
                                               sorted(filtered_df['訂單寬度'].unique() if '訂單寬度' in filtered_df.columns else []),
                                               key="comp_width")
                
                with comp_filter_col3:
                    comp_mat = st.multiselect("熱軋材質",
                                             sorted(filtered_df['熱軋材質'].unique() if '熱軋材質' in filtered_df.columns else []),
                                             key="comp_mat")
                
                with comp_filter_col4:
                    comp_spec = st.multiselect("產品規格代碼",
                                              sorted(filtered_df['產品規格代碼'].unique() if '產品規格代碼' in filtered_df.columns else []),
                                              key="comp_spec")
